...
public class ReservasActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    private void loadReservas(int usuarioId) {
        ...
        // Se realiza la llamada HTTP y se procesa la respuesta
        call.enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful() && response.body() != null) {
                    try {
                        // Se obtiene el cuerpo de la respuesta (formato JSON), se extrae el array "reservas"
                        // y se convierte en una lista de objetos Reserva usando Gson ( convertir objetos Java a JSON )
                        ...
                        // Se actualiza el adaptador con las reservas obtenidas
            ...
            public void onFailure(Call<JsonObject> call, Throwable t) {
                // Se muestra un error si falla la conexión con la API
